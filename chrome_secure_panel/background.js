...
const ELEMENT_PICKER_PAGE_EVENT = "assistant.browser.element_picker.page_event";
const ELEMENT_PICKER_RESULT_EVENT = "assistant.browser.element_picker.result";
const ELEMENT_PICKER_CANCELLED_EVENT = "assistant.browser.element_picker.cancelled";
const ELEMENT_PICKER_ERROR_EVENT = "assistant.browser.element_picker.error";
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  if (!activePicker || activePicker.tabId !== tabId) {
    return;
  }
  if (typeof changeInfo.url === "string" || changeInfo.status === "loading") {
    void cancelActivePicker({ reason: "tab_updated", notify: true });
  }
});

chrome.tabs.onActivated.addListener((activeInfo) => {
  if (!activePicker || activeInfo?.tabId === activePicker.tabId) {
    return;
  }
  void cancelActivePicker({ reason: "tab_switched", notify: true });
});
...
chrome.tabs.onRemoved.addListener((tabId) => {
  ...
  emitElementPickerEvent(ELEMENT_PICKER_CANCELLED_EVENT, {
    tabId,
    url: current?.url || "",
    reason: "tab_closed"
  });
...
async function handleMessage(message, sender = null) {
  ...
  if (
    message.type === ELEMENT_PICKER_RESULT_EVENT
    || message.type === ELEMENT_PICKER_CANCELLED_EVENT
    || message.type === ELEMENT_PICKER_ERROR_EVENT
  ) {
    return { acknowledged: true };
  }
  if (message.type === ELEMENT_PICKER_PAGE_EVENT) {
    return await handleElementPickerPageEvent(message, sender);
  }
  ...
  if (message.type === "assistant.browser.element_picker.start") {
    return await startElementPicker(message);
  }
  if (message.type === "assistant.browser.element_picker.cancel") {
    return await cancelElementPicker(message);
  }
  ...
  if (message.type === "assistant.tools.page_hosts.active_tab") {
    ...
    return {
      active_tab: {
        ...
        allowlisted: isHostAllowed(activeTab.url, snapshot.effective_allowed_hosts),
        active: true
      }
    };
  }
...
function getPickerActiveTabInfo(tab, allowedHosts) {
  ...
  const host = extractUrlHost(tab.url);
  if (!host) {
    throw new Error("Active tab host is invalid.");
  }
  if (!isHostAllowed(tab.url, allowedHosts)) {
    throw createHostNotAllowlistedError(tab.url, "Active tab is not in the extension allowlist.", allowedHosts);
  }
  return {
    tabId: tab.id,
    url: String(tab.url),
...
async function cleanupElementPickerInTab(tabId) {
  ...
  try {
    await runInTab(tabId, clearElementPickerInPage, []);
  } catch {
...
async function startElementPicker(_message) {
  ...
  await ensureUrlHostPermission(activeInfo.url);
...
async function handleElementPickerPageEvent(message, sender) {
  ...
  if (eventType === "result") {
    emitElementPickerEvent(ELEMENT_PICKER_RESULT_EVENT, {
      payload: {
        ...payload,
  ...
  emitElementPickerEvent(ELEMENT_PICKER_ERROR_EVENT, {
    tabId: current.tabId,
    url: current.url,
...
async function buildAssistantBrokerPayload(message) {
  ...
  if (browserRuntimeRequested) {
    const runtimeTab = await getActiveTab();
    if (runtimeTab?.id && runtimeTab.url) {
      const host = extractUrlHost(runtimeTab.url);
      browserRuntimeContext = {
        tabId: runtimeTab.id,
        url: String(runtimeTab.url),
        title: String(runtimeTab.title || ""),
        host: host || "",
        allowlisted: host ? isHostAllowed(runtimeTab.url) : false,
        active: true
      };
    }
  }
...
function clearElementPickerInPage() {
